    ...
    + dharitri_sc_modules::ongoing_operation::OngoingOperationModule
{
    ...
    fn perform_mex_operations_endpoint(&self, service_index: usize) -> OperationCompletionStatus {
        ...
        let _ = self.run_while_it_has_gas(0, || {
            ...
            let token_id = if fee.fees.token_identifier.is_moax() {
                // TODO: Wrap moax and take cost into consideration
                TokenIdentifier::from("PLACEHOLDER")
            } else {
                fee.fees.token_identifier.unwrap_dct()
            };
    ...
    fn perform_mex_operations(
        ...
        if actions_value.fees > 0 {
            self.total_fees().update(|fees| {
                fees.add_payment(DctTokenPayment::new(
                    token_id.clone(),
                    0,
        ...
        if mex_to_burn > 0 {
            self.send()
                .dct_local_burn(&bought_mex.token_identifier, 0, &mex_to_burn);
        }
        ...
        let _ = self.call_lock_tokens(
            simple_lock_address,
            DctTokenPayment::new(bought_mex.token_identifier, 0, mex_to_lock),
            lock_period,
            user_address,
        );
    }

    fn buy_mex(&self, token_id: TokenIdentifier, amount: BigUint) -> DctTokenPayment {
    ...
    fn call_swap_to_mex(
        ...
        self.other_pair_proxy(pair_address)
            ...
            .with_dct_transfer(DctTokenPayment::new(input_token_id, 0, amount))
