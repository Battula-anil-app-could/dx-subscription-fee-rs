    ...
    + dharitri_sc_modules::ongoing_operation::OngoingOperationModule
{
    ...
    fn subtract_payment(
        ...
        service_index: usize,
        user_id: AddressId,
    ) -> MyVeryOwnScResult<MoaxOrDctTokenPayment, ()> {
    ...
    fn subtract_specific_token(
        ...
        if token_id.is_moax() {
            return self.user_deposited_moax(user_id).update(|moax_value| {
                if *moax_value < amount {
                    return MyVeryOwnScResult::Err(());
                }

                *moax_value -= &amount;

                MyVeryOwnScResult::Ok(MoaxOrDctTokenPayment::new(
        ...
        let payment = DctTokenPayment::new(token_id.unwrap_dct(), 0, amount);
        let raw_result = self
    ...
    fn subtract_any_token(
        ...
        for i in 0..user_tokens.len() {
            ...
            return MyVeryOwnScResult::Ok(MoaxOrDctTokenPayment::new(
