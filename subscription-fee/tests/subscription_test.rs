...
fn subscribe_before_approve_test() {
    ...
    b_mock_rc
        .borrow_mut()
        .set_dct_balance(&user, FIRST_TOKEN_ID, &rust_biguint!(1_000_000));
...
fn subtract_ok_test() {
    ...
    b_mock_rc.borrow().check_dct_balance(
        &rand_service,
        FIRST_TOKEN_ID,
...
fn withdraw_tokens_test() {
    ...
    b_mock_rc
        .borrow()
        .check_dct_balance(&user, FIRST_TOKEN_ID, &rust_biguint!(999_999));
    ...
    b_mock_rc
        .borrow()
        .check_dct_balance(&rand_service, FIRST_TOKEN_ID, &rust_biguint!(0));
}
