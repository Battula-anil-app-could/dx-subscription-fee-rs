...
use dharitri_sc::types::{
    Address, MoaxOrDctTokenIdentifier, MultiValueEncoded, TokenIdentifier,
};
    ...
    SubscriptionObjBuilder: 'static + Copy + Fn() -> subscription_fee::ContractObj<DebugApi>,
{
        ...
        b_mock
            ...
            .execute_tx(owner_addr, &s_wrapper, &rust_zero, |sc| {
                ...
                for arg in accepted_tokens {
                    if &arg == b"MOAX" {
                        let token_id = MoaxOrDctTokenIdentifier::moax();
                        args.push(token_id);
                    } else {
                        let token_id = TokenIdentifier::from_dct_bytes(arg);
                        args.push(MoaxOrDctTokenIdentifier::dct(token_id));
                    }
                }
    ...
    pub fn call_deposit(&mut self, caller: &Address, token_id: &[u8], amount: u64) -> TxResult {
        self.b_mock.borrow_mut().execute_dct_transfer(
            caller,
            &self.s_wrapper,
            token_id,
            0,
